...
def main(run_dir, n_images, split, checkpoint, threshold, matching_radius, fp_only, r_interclass):
    ...
    suffix = f'error_vis_{split}' if not fp_only else f'fp_vis_{split}'
    ...
    with torch.no_grad():
        for idx, (img_batch, _, pos_pts_tuple, neg_pts_tuple) in enumerate(loader):
            ...
            if fp_only:
                ...
                ax1.set_title(f'Positive FP | n={len(fp_pos)}')
                ax1.legend()
                ax1.axis('off')

                ax2.imshow(img_np)
                scatter(ax2, fp_neg, 'red', 'FP')
                ax2.set_title(f'Negative FP | n={len(fp_neg)}')
                ax2.legend()
                ax2.axis('off')

                fig.savefig(out_dir / f'image_{idx:03d}_fp.png', dpi=150, bbox_inches='tight')
                plt.close(fig)
                print(f"Saved image_{idx:03d}_fp.png")
            else:
                # existing full visualization (TP/FP/FN + GT plots)
                fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--matching_radius', type=float, default=20.0)
    parser.add_argument('--fp_only', action='store_true', help='Only draw false positive predictions (no GT match)')
    parser.add_argument('--r_interclass', default=15, type=float)
    args = parser.parse_args()
    main(Path(args.run_dir), args.n_images, args.split, args.checkpoint, args.threshold, args.matching_radius, args.fp_only, args.r_interclass)
