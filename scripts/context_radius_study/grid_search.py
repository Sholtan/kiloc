...
def run_one(i, combo, param_names, base_cfg, gridrun_name, grid_runs_dir, gpu_id):
    ...
    log_dir = Path("checkpoints") / "context_radius" / gridrun_name / "logs"
    ...
    cmd = [
        "python", "scripts/context_radius_study/run_train.py",
        "--config", str(temp_config_path),
        "--out_dir", f"context_radius/{gridrun_name}",
        "--run_name", run_name,
    ]
    ...
    return {
        ...
        "f1_pos": best["f1_pos"],
        "precision_neg": best["precision_neg"],
        "recall_neg": best["recall_neg"],
        "f1_neg": best["f1_neg"],
    }
...
def main(grid_config_path, gpus):
        ...
        for fut in as_completed(futures):
            ...
            if res["status"] == "ok":
                print(
                    ...
                    f"F1_pos={res['f1_pos']:.3f} "
                    f"F1_neg={res['f1_neg']:.3f} "
    ...
    for r in ok_results:
        print(
            ...
            f"F1_pos={r['f1_pos']:.3f} "
            f"F1_neg={r['f1_neg']:.3f} "
            f"| epoch={r['best_epoch']} "
            f"| gpu={r['gpu_id']} "
            f"| {r['params']}"
        )

    output_path = Path("checkpoints") / "context_radius" / gridrun_name / "grid_results.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--grid_config", default="configs/context_radius_study/grid_classifier.yaml")
